      ...
      <div className="flex flex-col items-center justify-center mb-8">
        ...
        <div className="flex-col flex gap-5">
          {Object.entries(skillCategory).map(([category, skills]) => (
            <div key={category} className="bg-gray-800/60 rounded-xl p-4 border border-[#30363d] hover:border-blue-500 hover:-translate-y-0.5 transition-all duration-300 hover:shadow-lg hover:shadow-blue-500/20">
              <div className="text-[#8d96a0] text-[18px] font-medium mb-4">
                <span className="bg-linear-to-r from-blue-400 to-purple-500 bg-clip-text text-transparent">{category}</span>
              </div>
              <div className="flex flex-wrap gap-2">
                {skills.map((skill) => {
                  return (
                    <div key={skill.title} className="flex items-center justify-center gap-1 rounded-md px-2 py-0.5 bg-[#43474d]">
