  ...
  return (
    <div id="experience" className='px-[2em] pt-10 pb-10 text-white flex flex-col scroll-mt-20'>
      <div className="flex flex-col items-center justify-center mb-8">
        <div className="text-center text-4xl md:text-5xl font-bold mb-2">{t('experience.title')}</div>
      ...
      <div className="relative mt-2 md:max-w-[min(800px,calc(100%-40px))] mx-auto w-full box-border">
        ...
        {Experiences.map((experience) => (
          <FadeInSection key={experience.titlekey}>
            <div className="flex-1 group my-8 backdrop-blur-lg bg-gray-800/60 border border-white/10 rounded-3xl p-6 transition-all duration-500 hover:-translate-y-3 hover:border-blue-500 hover:shadow-2xl hover:shadow-blue-500/30 ml-7">
              {/* Experience Dot */}
              ...
              <div className="text-xl font-bold text-white mb-2 relative z-10">{t(experience.titlekey)}</div>
              ...
              <div className="text-base text-gray-400 font-medium mb-4 relative z-10">{t(experience.rolekey)}</div>
              <span className="inline-block bg-blue-400/20 text-blue-200 px-4 py-2 rounded-full text-sm font-medium mb-5 border border-blue-500/30 relative z-10">{t(experience.timekey)}</span>
              <p className="text-gray-300 leading-relaxed mb-5 relative z-10">{t(experience.descriptionkey)}</p>
